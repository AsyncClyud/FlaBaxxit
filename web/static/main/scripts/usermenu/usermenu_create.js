...
async function UserMenu() {
  ...
  if(await IsAuth() == 0){
    ...
    login_element.setAttribute("class", "bg-[#ebebeb] text-[black] dark:bg-[#2a323c] dark:text-[gainsboro] rounded-[3px] p-[5px] m-[5px]")
    login_element.href = `/auth/login`
    login_element.textContent = `Log in`

    const register_element = document.createElement("a")
    register_element.setAttribute("class", "bg-[#ebebeb] text-[black] dark:bg-[#2a323c] dark:text-[gainsboro] rounded-[3px] p-[5px] m-[5px]")
